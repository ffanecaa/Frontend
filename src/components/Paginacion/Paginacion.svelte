<script>
  ...
  async function traerDatos() {
    const resposta = await fetch(
      `http://localhost:8000/pax/?page=${page}&limit=${limit}`
    );
    const datos = await resposta.json();
    return datos;
  }
  function traerDatosm() {
    traerDatos().then((datosrecibidos) => {
      info = datosrecibidos;
      console.log(info);
    });
  }
  ...
  function mas() {
    traerpaxPosterior().then((datosrecibidos) => (info = datosrecibidos));
    console.log(info);
  }
  //------------------PREVIOUS PAGE --------ENVIO RUTA--------ºº-----------------------------------
  async function traerPaxAnterior() {
    const resposta = fetch(
      `http://localhost:8000${info.pagination.previousPage}`
    );
    const datos = (await resposta).json();
    return datos;
  }
  ...
  function menos() {
    traerPaxAnterior().then((datosrecibidos) => (info = datosrecibidos));
    console.log(info);
  }
</script>
...
<div class="container_pagination">
   <div class="traer">
    <input type="text" placeholder=" página" bind:value={page} />
    <!-- elementos -->
